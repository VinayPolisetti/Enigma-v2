...
class Dashboard extends Component {
  ...
  async getfiles() {
    ...
    for (let i = 0; i < this.state.files["0"].length; ++i) {
      ...
      name=await userobj.methods.username().call();
      }
      var has = [];
  ...
  async componentDidMount() {
    ...
    const result = await globalfile.methods.getglobalrequests().call();
      result[0].forEach((message, index) => {
        addResponseMessage(`${message} (from ${result[1][index]})`);
        this.setState(prevState => ({
          messages: [...prevState.messages, { author: 'them', message }]
        }));
      });

      setInterval(async () => {
        const newResult = await globalfile.methods.getglobalrequests().call();
        newResult[0].forEach((message, index) => {
          if (!this.state.messages.find(m => m.message === message)) {
            addResponseMessage(`${message} (from ${newResult[1][index]})`);
            this.setState(prevState => ({
              messages: [...prevState.messages, { author: 'them', message }]
            }));
          }
  ...
  handleNewUserMessage = async newMessage => {
    ...
    // Update the state of the messages array with the new message.
    this.setState(prevState => ({
        messages: [...prevState.messages, { author: 'me', message: newMessage }]
      }));
    ...
    return (
      <div class="wrapper" style={{ fontFamily: "sans-serif" }}>
        ...
        <Navbar bg="dark" variant="dark">
          ...
          <div style={{ position: "fixed", bottom: "0%", left: "0", zIndex: "9999", margin:"0%" }}>
  <Card style={{ margin: "0px 5px" }}>
    <br />
    <h4 className="font-weight-bold" style={{ fontFamily: "Arial", marginBottom: "0", marginLeft: "23px" }}>
      Upload from here
    </h4>
    <CardBody>
      ...
      <Form onSubmit={this.fileSubmitAnony}>
        ...
        >
          ...
          <div>
            <Container>
            <h3 style={{ marginTop: "7%", marginLeft: "20%" }}>
            <b>GLOBAL SPACE</b>
            </h3>
            </Container>
            {/* add files public */}

            <div style={{margin: "5% 20% 0% 30%"}}>
              {/* Text here */}
              <h5 className="font-weight-bold" style={{textAlign: "center"}}>Doc Files</h5>
              ...
              {/* pdf files */}
              <h5 className="font-weight-bold" style={{textAlign: "center"}}>PDF Files</h5>
              <div class="carousel js-carousel my-5">
                <div class="carousel__container js-carousel-container mx-4">
                  <div class="carousel__list js-carousel-list">
                    {this.state.showPdf}
              ...
              {/* images here */}
              <h5 className="font-weight-bold" style={{textAlign: "center"}}>Images</h5>
              <div class="carousel js-carousel my-5">
                <div class="carousel__container js-carousel-container mx-4">
                  <div class="carousel__list js-carousel-list">
              ...
              {/* videos here */}
              <h5 className="font-weight-bold" style={{textAlign: "center"}}>Videos</h5>
              <div class="carousel js-carousel my-5">
                <div class="carousel__container js-carousel-container mx-4">
                  <div class="carousel__list js-carousel-list">
          ...
          <Widget
              ...
              title="Global Request Box"
              subtitle="Ask for Anything"
              senderPlaceHolder="Type your message here..Press Enter to send"
            />
        </div>
      </div>
    );
