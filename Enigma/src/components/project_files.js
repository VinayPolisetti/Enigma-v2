          ...
          <tr>
            ...
            <td>
              <Button
                ...
                style={{ padding: "0.10rem 0.5rem", fontSize: "0.875rem" }}
              >
                Share
              </Button>
              </td>
...
sharefile =async(childaddr)=>{
  ...
  const projobj = await new web3.eth.Contract(projectabi, this.state.project);
  const nodeobj=await projobj.methods.nodeStructs(childaddr).call({from:accounts[0]});
  var sepub = await userobj.methods.publickey().call();
  console.log(nodeobj);
  var aesencr=nodeobj['aespass'];
  var aesdecr=quickEncrypt.decrypt(aesencr,this.state.privatekey);
  console.log(aesdecr);
  ...
  try {
    ...
    let seencr=quickEncrypt.encrypt(aesdecr,sepub);
    ...
    await userobj.methods.addsendfile(ipfsaddr,seencr,fname, enteredadd).send({
      from:accounts[0],
      file:ipfsaddr,
      aespass:aesencr,
      nm:fname,
...
render(){
    return (
      <div className="App">
          ...
          <h4 className="font-weight-bold mt-5 text-center">Group Files</h4>
          <div style={{marginLeft:'18.3%'}}>
            <a type="button" className="text-capitalize" style={{textDecoration:'none',float:'left',fontSize:'18px'}}>Current Location - {this.state.path}</a>
          </div>
          <div class="main-pge">
          <Row>
          <Col style={{ margin: "2% 3%" }}>
    ...
    <Col lg="6" className="text-center">
        <Form onSubmit={this.Encrypt}>
            ...
            <Button className="btn-sm-round my-3" style={{position:'absolute',left: '220px', bottom:'17px'}} color="primary">
                Add File
            </Button>
        </Form>
    </Col>
    <Col lg="6" className="text-center">
        <Form onSubmit={this.addfolder}>
            <div>
                <h6 className="font-weight-bold my-2">Create a New Folder</h6>
            </div>
            <Row className="justify-content-center">
                <Form.Group>
                    <Form.Control
            ...
            <Row>
              <Col
                className="table-sec" style={{marginLeft: "6%"}}
              >
                <Table bordered hover responsive stripped>
                  <tbody>{this.state.showfiles}</tbody>
                </Table>
              </Col>
              </Row>
              <form onSubmit={this.addchat}>
                              <h4 className="font-weight-bold my-4">Type Message here</h4>
                              <div>
                              ...
                              <h4 className="font-weight-bold my-4" >Group Chat Area</h4>
